...
- `home.nix` links `home/.config/` and `home/.claude/` into `$HOME` with `mkOutOfStoreSymlink`. Lazy.nvim and Pi write through those links, so their tracked runtime files - `home/.config/nvim/lazy-lock.json` and `home/.pi/agent/settings.json` - surface as unrelated diffs; leave them out of commits unless the task is about them. Claude Code differs: `/config` replaces the `~/.claude/settings.json` link with a regular file instead of writing through it, so those settings do not persist into this repo.
